...
File naming for story artifacts: `{YYYY-MM-DD}-{command-type}-{slug}.md` (e.g. `2026-05-29-plan-auth-flow.md`).

Story path is provided by the `bob:story-context` skill, which every engineering command invokes via the context-protocol. Do not hardcode `ai/` paths in new engineering commands.
